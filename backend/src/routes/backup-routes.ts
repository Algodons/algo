 ...
 */
router.post(
  '/connections/:connectionId/backups',
 ...
 */
router.post(
  '/connections/:connectionId/backups/:backupId/restore',
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { connectionId, backupId } = req.params;
 ...
 */
router.delete(
  '/connections/:connectionId/backups/:backupId',
 ...
 */
router.get(
  '/connections/:connectionId/backups/:backupId/download',
  (req: Request, res: Response): void => {
    try {
      const { connectionId, backupId } = req.params;
      const backup = backupService.getBackup(connectionId, backupId);
      ...
      res.download(
        backup.path,
        `${backupId}.${backup.format}${backup.compressed ? '.gz' : ''}`,
        (err) => {
          if (err) {
 ...
 */
router.put(
  '/connections/:connectionId/backups/schedules/:scheduleId',
  (req: Request, res: Response): void => {
    try {
 ...
 */
router.post(
  '/connections/:connectionId/backups/apply-retention',
  (req: Request, res: Response): void => {
    try {
      const { connectionId } = req.params;
 ...
 */
router.post(
  '/connections/:connectionId/backups/pitr',
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { connectionId } = req.params;
