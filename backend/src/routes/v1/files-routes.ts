...
export function createFilesRoutes(pool: Pool): Router {
  ...
  function validatePath(
    filePath: string,
    projectId: string
  ): { isValid: boolean; fullPath: string } {
  ...
  router.post('/*', authenticate(pool), async (req: Request, res: Response) => {
    const filePath = req.params[0] || '';
  ...
  router.put('/*', authenticate(pool), async (req: Request, res: Response) => {
    const filePath = req.params[0] || '';
  ...
  router.delete('/*', authenticate(pool), async (req: Request, res: Response) => {
    const filePath = req.params[0] || '';
    const projectId = req.query.projectId as string;
